...
const getProjectClassName = (index: number) => {
  switch (index) {
    case 0:
...
export const App = () => {
  return (
    <div className={styles.mainContainer}>
      <div className={styles.loopContainer1}>
        <Loops />
      </div>
      <div className={styles.loopContainer2}>
        <Loops />
      </div>
      <div className={styles.appContainer}>
        ...
        <section className={styles.projectContainerMain}>
          {projects.map((val, idx) => {
            return (
              <div key={idx} className={getProjectClassName(idx)}>
                <Projects
                  image={val.image}
                  title={val.header}
                  skills={val.technology}
                />
              </div>
            );
          })}
      ...
      <section className={styles.contactSectionContainer}>
        <div className={styles.contactFormContainer}>
          <header className={styles.contactHeader}>Contact</header>
          <div className={styles.cta}>{introduction.cta}</div>
          <div className={styles.contactLoops}>
            <Loops/>
          </div>
          <form className={styles.contactForm}>
          <input className={styles.input} type={'text'} placeholder="FREDRICK JOULES"/>
          <input className={styles.input} type={'email'} placeholder='FREDRICK@EXAMPLE.COM'/>
          <div></div>
